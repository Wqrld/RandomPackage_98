	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		if(!(sender instanceof Player)) {
			...
			for(int i = 1; i <= 3; i++) {
				...
				if(RandomPackage.getEnchanterConfig().getString("EnchanterGui." + iteM + ".Enabled").equalsIgnoreCase("true")) {
			...
			} else if(RandomPackage.getEnchanterConfig().getString("DefaultGui").equalsIgnoreCase("Other")) {
				player.openInventory(enchanterListener.other);
			} else if(RandomPackage.getEnchanterConfig().getString("DefaultGui").equalsIgnoreCase("SoulTrackers")) {
